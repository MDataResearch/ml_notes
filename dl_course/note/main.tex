...
\noindent To train the model, we need to update \(W\) for each epoch: (gradient decent)

\[W := W - \alpha  \frac{\partial}{\partial W} J(W, B)\]

\noindent We can see \(\frac{\partial}{\partial W} J(W, B)\) is hard to get directly. There is an easier way to calculate it. For each training example \((x^{(q)}, y^{(q)}), q \in \{1, \dots, m\}\), define cost function:

\[J = -\sum_{k = 1}^{K} [y^{(q)}_k log(h_{W} (x^{(q)})_k) + (1 - y^{(q)}_k) log(1 - h_{W}(x^{(q)})_k)] \]

\noindent Apply chain rule we have:

\[\frac{\partial J}{\partial W_{ij}^{(l)}} = \frac{\partial J}{\partial z^{(l + 1)}} (a^{(l)})^T\]
...
& = \frac{\partial}{\partial z_p^{(L)}} \sum_{k = 1}^{K} -[y^{(q)}_k log(h_{W} (x^{(q)})_k) + (1 - y^{(q)}_k) log(1 - h_{W}(x^{(q)})_k)] \\
...
\[\frac{\partial J}{\partial W_{ij}^{(l)}} = \delta^{(l + 1)} (a^{(l)})^T\]
\[\delta^{(l)} = (W^{(l)})^T \delta^{(l + 1)} \odot (a^{(l)} \odot (1 - a^{(l)}))\]
\[\delta^{(L)} = a^{(L)} - y^{(q)}\]

\noindent \textbf{backpropagation algorithm:} (compute \(\frac{\partial}{\partial W} J(W, B)\))
...
\noindent compute \(\frac{\partial}{\partial W_{ij}^{(l)}} J(W, B) = D_{ij}^{(l)} = 
\begin{cases}
  \frac{1}{m} \Delta_{ij}^{(l)} + \lambda W_{ij}^{(l)}, & \text{if } j \neq 0 \\
