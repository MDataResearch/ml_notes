...
\noindent We can see \(\frac{\partial}{\partial W} J(W)\) is hard to get directly. There is an easier way to calculate it. For each training example \((x^{(q)}, y^{(q)}), q \in \{1, \dots, m\}\), define cost function:
...
\noindent \textbf{backpropagation algorithm:} (compute \(\frac{\partial}{\partial W} J(W)\))
...
\noindent compute \(\frac{\partial}{\partial W_{ij}^{(l)}} J(W) = D_{ij}^{(l)} = 
\begin{cases}
