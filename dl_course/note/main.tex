...
\noindent Assume the original image is of shape \((100, 100, 3)\), then the image is divided into \(3 \times 3\) regions (or \(19 \times 19\) which avoids multi-objects in one region). Apply Object Localization algorithm to each of the region by using a CNN, in the end we get predictions for all the regions in the final output, which is of shape \((3, 3, 8)\).

\bigskip

\noindent Notice that \(b_{x}, b_{y}, b_{h}, b_{w}\) should be the percentage (as decimals) of each divided region.

\bigskip

\noindent One of the best advantages that makes the YOLO algorithm popular is that it has a great speed and a Conv net implementation. But it will have a problem when there are more than one objects in one divided region.
