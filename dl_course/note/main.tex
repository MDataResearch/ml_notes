...
\noindent set \(\Delta_{ij}^{[l]} = 0, l \in \{0, \dots, L - 1\}\)
...
\noindent \hspace{1cm} compute \(\Delta^{[l]} := \Delta^{(l)} + \frac{\partial J}{\partial W^{[l]}} = \Delta^{[l]} + \delta^{[l + 1]}(a^{[l]})^T\)

\noindent compute \(\frac{\partial}{\partial W_{ij}^{[l]}} J(W, B) = D_{ij}^{[l]} = \frac{1}{m} \Delta_{ij}^{[l]} + \frac{\lambda}{m} W_{ij}^{[l]}
\)
