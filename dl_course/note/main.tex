...
\noindent \textbf{rectified linear unit:}
...
\[J(w, b) = \frac{1}{m} \sum^m_{i = 1} L(\hat{y}^{(i)}, y^{(i)}) = - \frac{1}{m} \sum^m_{i = 1} [ylog(\hat{y}) + (1 - y)log(1 - \hat{y})]\]

\noindent \textbf{gradient descent:} find best \(w\), \(b\) to minimize \(J(w, b)\)
