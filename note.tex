...
\noindent Recall that if \(m < n\), then \(\textbf{X}^T\textbf{X}\) is non-invertible. However, when we add the term \(\lambda \textbf{L}\), the term \(\textbf{X}^T\textbf{X} + \lambda \textbf{L}\) becomes invertible.
