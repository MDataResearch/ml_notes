...
\noindent We can see \(\frac{\partial}{\partial \Theta} J(\Theta)\) is hard to get directly. And there is an easier way to calculate \(\frac{\partial}{\partial \Theta^{(l)}} J(\Theta^{(l)})\) for each layer \(l\) in \(1 \leq l \leq L - 1\):

\begin{equation*}
\begin{split}
\frac{\partial}{\partial \Theta^{(l)}} J(\Theta^{(l)})
& = \frac{\partial}{\partial z^{(l + 1)}} J(\Theta^{(l)}) \cdot \frac{\partial}{\partial \Theta^{(l)}} z^{(l + 1)} \\
& = \frac{\partial}{\partial z^{(l + 1)}} J(\Theta^{(l)}) \cdot (a^{(l)})^T
\end{split}
\end{equation*}

\noindent for \(1 \leq l \leq L - 1\): (to be fixed)

\begin{equation*}
\begin{split}
\frac{\partial}{\partial z^{(l)}} J(\Theta^{(l)})
& = \frac{\partial}{\partial z^{(l + 1)}} J(\Theta^{(l)}) \cdot \frac{\partial}{\partial a^{(l)}} z^{(l + 1)} \cdot \frac{\partial}{\partial z^{(l)}} a^{(l)} \\
& = \frac{\partial}{\partial z^{(l + 1)}} J(\Theta^{(l)}) \cdot (\Theta^{(l)})^T \cdot g'(z^{(l)}) \\
& = \frac{\partial}{\partial z^{(l + 1)}} J(\Theta^{(l)}) \cdot (\Theta^{(l)})^T \cdot a^{(l)} \cdot (1 - a^{(l)})
\end{split}
\end{equation*}
